import { mixRound, benchHistory, pairHistory } from "../worker/balance.js";
...
/* --- shape ---------------------------------------------------------- */
{
  ...
  t("sides close on skill", r.gap <= 1, `gap=${r.gap}`);
}
{
...
}
{
  ...
  t("every round stayed balanced", history.every((r) => r.gap <= 2),
...
/* --- explicit side size --------------------------------------------- */
{
  ...
  t("still balanced at 4 v 4", r.gap <= 2, `gap=${r.gap}`);
...

{
  ...
  t("sides stayed balanced across the session",
    history.every((r) => r.gap <= 2), history.map((r) => r.gap).join(","));
}
...
/* --- integrity ------------------------------------------------------ */
{
  ...
  for (let i = 0; i < 200; i++) {
    ...
    checked++;
    const players = mk(Array.from({ length: size }, () => "AIB"[Math.floor(Math.random() * 3)]));
    ...
    if (new Set(ids).size !== size || ids.length !== size ||
        new Set(sizes).size !== 1 || r.gap > 3) bad++;
  }
  t("random rosters and side sizes: nobody lost or cloned, gap <= 3", bad === 0,
